...
try:
    ...
    # Simulate the exact map data endpoint logic
    print("=== Testing Map Data Endpoint Logic ===")
    
    # Get crew locations and assignments
    requested_types = ["crews"]
    map_data = {}
    
    if "crews" in requested_types:
        print("Processing crews section...")
        # Check if database has real data, otherwise return empty
        try:
            print("Executing crew count query...")
            ...
            if crew_count > 0:
                ...
                for row in results:
                    ...
                    print('Added crew:', crew_data['name'])
                
                print('Total processed crews:', len(crews))
                map_data["crews"] = crews
                print('Set map_data["crews"] =', len(map_data["crews"]), 'crews')
            else:
            ...
            print('Exception in crews section:', e)
            import traceback
            traceback.print_exc()
    ...
    print('map_data["crews"] length:', len(map_data.get("crews", [])))
    if map_data.get("crews"):
        print('First crew:', map_data["crews"][0])
